                ...
                {images.map(img => (
                    <li key={img._id} className="relative group">
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                            <button onClick={() => openEdit(img)} className="text-xs px-2 py-1 bg-white rounded">편집</button>
...
const EditDialog = forwardRef<HTMLDialogElement, {
    ...
    return (
        <dialog ref={ref} className="rounded p-0 backdrop:bg-black/50 w-full max-w-md">
            <form method="dialog" className="p-6 space-y-4">
                <h2 className="font-medium">사진 편집</h2>
                <input value={title} onChange={e => setTitle(e.currentTarget.value)} placeholder="제목" className="w-full border-b border-neutral-300 pb-1 text-sm outline-none focus:border-neutral-700" />
                <textarea value={description} onChange={e => setDescription(e.currentTarget.value)} placeholder="설명" rows={3} className="w-full border border-neutral-300 rounded p-2 text-sm outline-none focus:border-neutral-700" />
                <input value={tags} onChange={e => setTags(e.currentTarget.value)} placeholder="태그 (공백 구분)" className="w-full border-b border-neutral-300 pb-1 text-xs outline-none focus:border-neutral-700" />
                ...
                <div className="flex justify-end gap-2 pt-2">
                    <button type="button" onClick={onCancel} className="text-sm px-3 py-1.5 text-neutral-500">취소</button>
                    <button
                        type="button"
                        onClick={() => onSave({ title, description, tags: tags.split(/\s+/).filter(Boolean), visibility })}
                        className="text-sm px-3 py-1.5 bg-neutral-900 text-white rounded"
                    >
                        저장
                    </button>
                </div>
...
const DeleteDialog = forwardRef<HTMLDialogElement, {
    ...
    return (
        <dialog ref={ref} className="rounded p-0 backdrop:bg-black/50 w-full max-w-sm">
            <div className="p-6 space-y-4">
                ...
                <div className="flex justify-end gap-2">
                    <button type="button" onClick={onCancel} className="text-sm px-3 py-1.5 text-neutral-500">취소</button>
                    <button type="button" onClick={onConfirm} className="text-sm px-3 py-1.5 bg-red-600 text-white rounded">삭제</button>
                </div>
            </div>
        </dialog>
    );
